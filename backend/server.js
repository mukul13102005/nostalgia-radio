...
const MIN_MSG_INTERVAL_MS = 400; // basic per-socket flood guard
const chatHistory = {};   // { themeId: [ {name, text, ts}, ... ] }
const roomUsers = {};     // { themeId: Set(socket.id) }
const socketMeta = {};    // { socket.id: { theme, name, lastMsgAt } }
...
io.on("connection", (socket) => {
  ...
  socket.on("chat message", ({ text }) => {
    // The room a message is posted to (and the name it's posted under) is
    // always taken from server-side state set by "join", never trusted
    // from the client payload — otherwise any connected client could emit
    // into a theme/private room it never actually joined (spam/spoofing
    // into rooms it has no code for), or post under a different name than
    // the one it joined with.
    const meta = socketMeta[socket.id];
    const cleanText = typeof text === "string" ? text.trim() : "";
    if (!meta || !meta.theme || !cleanText) return;

    const now = Date.now();
    if (meta.lastMsgAt && now - meta.lastMsgAt < MIN_MSG_INTERVAL_MS) return;
    meta.lastMsgAt = now;

    const theme = meta.theme;
    ensureTheme(theme);
    const msg = { name: meta.name || "Guest", text: cleanText.slice(0, 200), ts: now };
